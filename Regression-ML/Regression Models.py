...
mseL = sum((data['R'] - pRl)**2)/data['R'].size
...
plt.plot(data['X'],pRq,'*')
mseQ = sum((data['R'] - pRq)**2)/data['R'].size
...
A2 = np.array([[data.shape[0], sum(data['X']), sum(data['X']**2), sum(data['X']**3)],
...
plt.plot(data['X'],pRc,'*')
mseC = sum((data['R'] - pRc)**2)/data['R'].size
